...
void MainWindow::on_ajouter_clicked()
{
    ...
    // Input validation
    QRegularExpression nameRx("^[a-zA-Z]+$");        // Only letters
    QRegularExpression telRx("^\\d+$");              // Only digits
    QRegularExpression emailRx("^[\\w.-]+@[\\w-]+\\.[a-zA-Z]{2,}$"); // Email format

    if (!nameRx.match(nom).hasMatch() || !nameRx.match(prenom).hasMatch()) {
        QMessageBox::critical(this, tr("Erreur"), tr("Le nom et le prénom doivent contenir uniquement des lettres."), QMessageBox::Cancel);
    } else if (!telRx.match(telStr).hasMatch()) {
        ...
        if (client.Ajouterclient()) {
            ...
            if (query.next()) {
                ...
                if (id_client % 5 == 0) {
                    ...
                    label->setGeometry((width() - 600) / 2, (height() - 200) / 2, 600, 200);
                    label->show();
